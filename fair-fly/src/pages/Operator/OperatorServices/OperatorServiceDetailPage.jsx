...
export default function OperatorServiceDetailPage() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        console.warn('Error loading workflow templates in OperatorServiceDetailPage:', err);
      }
    );
    return () => unsubscribe();
  }, []);
              ...
              <div className="service-kpi-info">
                ...
                <span className="service-kpi-val">
                  {attachedWorkflows.length > 0
                    ? `${attachedWorkflows.length} (${totalStepsCount} Steps)`
            ...
            <article className="card detail-panel">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.875rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                <h2 className="panel-title" style={{ margin: 0 }}>
                  <i className="fa-solid fa-diagram-project"></i> Workflow Checklist Stages ({attachedWorkflows.length > 0 ? attachedWorkflows.length : (service.steps?.length ? 1 : 0)})
                </h2>
                {attachedWorkflows.length > 0 && (
                  <span className="status-pill status-active" style={{ fontSize: '0.75rem' }}>
                    {totalStepsCount} Total Stage{totalStepsCount !== 1 ? 's' : ''}
                  </span>
                )}
              </div>
                    ...
                    <div key={wf.id || wfIdx} className="attached-workflow-card">
                      <div className="attached-workflow-header">
                        ...
                        <span className="status-pill status-active" style={{ fontSize: '0.6875rem' }}>
                          {Array.isArray(wf.steps) ? wf.steps.length : 0} Stage{Array.isArray(wf.steps) && wf.steps.length === 1 ? '' : 's'}
                        </span>
                      </div>
                      ...
                      <div className="detail-workflows-list">
                        {Array.isArray(wf.steps) && wf.steps.length > 0 ? (
                          wf.steps.map((step, sIdx) => {
                            const stepTitle = typeof step === 'string' ? step : (step.title || step.name || `Stage ${sIdx + 1}`);
                            ...
                            const fileName = typeof stepFile === 'object' ? (stepFile?.name || 'Attached Guide/Document') : 'Attached Guide/Document';
